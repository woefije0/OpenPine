...
// Full tickerid form of the current symbol (e.g. "HYPERLIQUID:BTCUSD.P") -- factored out of the
// syminfo.tickerid builtin so request.security()/request.security_lower_tf() can compare a
// requested symbol argument against it (see pineSecurityIsSameSymbol below) without duplicating
// the string.
// 현재 심볼의 전체 tickerid 형태(예: "HYPERLIQUID:BTCUSD.P") — syminfo.tickerid 내장값에서
// 빼내서, request.security()/request.security_lower_tf()가 요청받은 심볼 인자를 이거랑
// 비교할 때(아래 pineSecurityIsSameSymbol) 문자열을 중복해서 안 만들어도 되게 했다.
function pineSymTickerId(){
  const t = pineSymTicker();
  return t ? 'HYPERLIQUID:' + t : '';
}
