...
const PINE_DYNAMIC_CONST_NS = {
  ...
  'syminfo.tickerid': () => pineSymTickerId(),
  'syminfo.prefix': () => PineHost.exchangePrefix,
...
// Full tickerid form of the current symbol (e.g. "HYPERLIQUID:BTCUSD.P", or just "BTCUSD.P" if the
// host never set an exchangePrefix) -- factored out of the syminfo.tickerid builtin so
// request.security()/request.security_lower_tf() can compare a requested symbol argument against
// it (see pineSecurityIsSameSymbol below) without duplicating the string.
// 현재 심볼의 전체 tickerid 형태(예: "HYPERLIQUID:BTCUSD.P", 호스트가 exchangePrefix를 안
// 정했으면 그냥 "BTCUSD.P") — syminfo.tickerid 내장값에서 빼내서, request.security()/
// request.security_lower_tf()가 요청받은 심볼 인자를 이거랑 비교할 때(아래
// pineSecurityIsSameSymbol) 문자열을 중복해서 안 만들어도 되게 했다.
function pineSymTickerId(){
  const t = pineSymTicker();
  if(!t) return '';
  return PineHost.exchangePrefix ? PineHost.exchangePrefix + ':' + t : t;
}
// The minimum price step for whichever symbol is currently "active" -- it.activeSecuritySymbol,
// set by buildSecuritySeries()/buildLowerTfArraySeries() while evaluating a request.security()/
// request.security_lower_tf() expression for a different symbol, or the current chart's own
// syminfo.ticker() the rest of the time. If the host registered a mintickResolver (pine-host.js)
// and it returns a positive number for that symbol, that's used as-is. Otherwise this falls back
// to a fixed 5-significant-figure approximation (matching Hyperliquid's convention) applied to
// whatever series is currently active (this chart's own bars, or the swapped-in other symbol's
// synthesized bars) -- which, for a symbol this engine has no explicit tick data for, is only
// ever a guess.
// 지금 "활성" 상태인 심볼(it.activeSecuritySymbol — request.security()/request.security_lower_tf()가
// 다른 심볼의 식을 평가하는 동안엔 buildSecuritySeries()/buildLowerTfArraySeries()가 설정하고,
// 그 외엔 이 차트 자신의 syminfo.ticker())의 최소 가격 단위. 호스트가 mintickResolver를
// (pine-host.js) 등록해뒀고 그게 이 심볼에 대해 양수를 돌려주면 그 값을 그대로 쓴다. 아니면
// 고정된 유효숫자 5자리 근사(하이퍼리퀴드 관례와 일치)를 지금 활성화된 시리즈(이 차트 자신의
// 봉, 또는 바꿔 낀 다른 심볼의 합성 봉)에 적용한 값으로 폴백한다 — 엔진이 명시적인 틱 정보를
// 모르는 심볼이라면 이건 그저 추측일 뿐이다.
function pineSymMintick(it){
  const symbol = (it && it.activeSecuritySymbol) || pineSymTicker();
  if(PineHost.mintickResolver){
    const override = PineHost.mintickResolver(symbol);
...
// request.security()/request.security_lower_tf()'s symbol argument means "this chart's own
// symbol" when it's empty/omitted, when it matches syminfo.ticker or syminfo.tickerid for the
// current symbol (the overwhelmingly common `request.security(syminfo.tickerid, tf, expr)`
// pattern), or when it's a ticker.heikinashi() marker (still the same underlying symbol, just
// transformed). Anything else is treated as a request for a genuinely different symbol, which is
// only servable from options.lowerTfCache -- see resolveLowerTfBars/pineSecurityCacheKey below.
// request.security()/request.security_lower_tf()의 symbol 인자는, 비어있거나(생략) 현재 심볼의
// syminfo.ticker/syminfo.tickerid와 일치하거나(압도적으로 흔한 `request.security(syminfo.tickerid,
// tf, expr)` 패턴), ticker.heikinashi() 마커일 때(여전히 같은 심볼, 변환만 다름) "이 차트 자신의
// 심볼"로 취급한다. 그 외는 진짜 다른 심볼 요청으로 보고, options.lowerTfCache에서만 서빙
// 가능하다 — 아래 resolveLowerTfBars/pineSecurityCacheKey 참고.
function pineSecurityIsSameSymbol(symbolVal){
  if(symbolVal == null || symbolVal === '') return true;
  if(typeof symbolVal !== 'string') return true;
  if(symbolVal.startsWith(PINE_HA_TICKER_PREFIX)) return true;
  const ticker = pineSymTicker();
  return symbolVal === ticker || (!!ticker && symbolVal === pineSymTickerId());
}
// Builds the lowerTfCache key a host uses to supply a genuinely different symbol's bars at a
// given timeframe (same-symbol entries keep using a plain timeframe string, e.g. "1S" -- this
// composite form is only needed for another symbol).
// 다른 심볼의 봉을 특정 timeframe으로 lowerTfCache에 넣을 때 호스트가 쓰는 키를 만든다(같은
// 심볼용 항목은 그대로 "1S" 같은 평범한 timeframe 문자열을 키로 쓴다 — 이 조합형 키는 진짜
// 다른 심볼일 때만 필요하다).
function pineSecurityCacheKey(symbol, tf){ return symbol + '@' + tf; }
...
class PineInterpreter {
  ...
  run(bars, inputOverrides){
    ...
    this.activeSecuritySymbol = null; // set while evaluating a request.security()/request.security_lower_tf() expression for a different symbol -- see pineSymMintick / request.security() 등이 다른 심볼의 식을 평가하는 동안 설정됨 — pineSymMintick 참고
  ...
  evalRequestSecurity(node){
    ...
    if(typeof symbolVal === 'string' && symbolVal.startsWith(PINE_HA_TICKER_PREFIX)){
      ...
      try{ return this.evalSecurityWithCurrentSeries(node, exprNode, timeframeVal, lookaheadVal, symbolVal); }
      finally{ Object.assign(this, saved); }
    }
    return this.evalSecurityWithCurrentSeries(node, exprNode, timeframeVal, lookaheadVal, symbolVal);
  }
  ...
  evalSecurityWithCurrentSeries(node, exprNode, timeframeVal, lookaheadVal, symbolVal){
    ...
    if(tf === '' && pineSecurityIsSameSymbol(symbolVal)){
      const v = this.evalExpr(exprNode);
      return v === undefined ? null : v;
    }
    // A different symbol was requested at "the same resolution as this chart" (e.g. via
    // timeframe.period, tf === ''). There's no timeframe string here to align lowerTfCache
    // entries by (unlike every other symbol/timeframe combination below, which resolves through
    // resolveLowerTfBars), so synthesizing a calendar-correct match isn't safe -- this specific
    // combination isn't supported and returns na, same as any other unresolvable request.
    // "차트와 같은 해상도"로(예: timeframe.period, tf==='') 다른 심볼을 요청한 경우다. 이 아래
    // 다른 모든 심볼/timeframe 조합과 달리(resolveLowerTfBars로 풀림) 여기엔 lowerTfCache 항목을
    // 맞춰볼 timeframe 문자열이 없어서, 달력 기준으로 정확히 맞춰 합성하는 게 안전하지 않다 —
    // 이 조합만은 지원하지 않고 다른 해석 불가능한 요청과 마찬가지로 na를 돌려준다.
    if(tf === ''){
      return null;
    }
    const lookaheadOn = typeof lookaheadVal === 'string' && lookaheadVal.indexOf('lookahead_on') !== -1;
    const s = getState(this, node);
    if(!s.series) s.series = this.buildSecuritySeries(tf, exprNode, lookaheadOn, symbolVal);
  ...
  // Like security(), the symbol argument is checked against pineSecurityIsSameSymbol() -- when it
  // names a different symbol, resolveLowerTfBars() only looks at lowerTfCache entries keyed for
  // that symbol (see pineSecurityCacheKey).
  ...
  // 없는 데이터가 필요하다. 그 데이터는 this.lowerTfCache(timeframe 문자열 -> 봉 배열)에서
  // 동기로 조회만 하면 된다. symbol 인자도 security()처럼 pineSecurityIsSameSymbol()로 판정해서,
  // 다른 심볼이면 resolveLowerTfBars()가 그 심볼용으로 키가 잡힌 항목만 본다(pineSecurityCacheKey 참고).
  evalRequestSecurityLowerTf(node){
    const rawArgs = node.args;
    let timeframeVal = '', exprNode = null, symbolVal = null, posIdx = 0;
    for(const a of rawArgs){
      if(a.named){
        if(a.name === 'timeframe') timeframeVal = this.evalExpr(a.value);
        else if(a.name === 'expression') exprNode = a.value;
    ...
    const tf = String(timeframeVal || '');
    const s = getState(this, node);
    if(!s.arr) s.arr = this.buildLowerTfArraySeries(tf, exprNode, symbolVal);
  ...
  // symbolVal decides which lowerTfCache entries are even eligible: for the current chart's own
  // symbol (pineSecurityIsSameSymbol), only plain-timeframe-string keys ("1S", "5", "1D", ...)
  // are considered, exactly as before. For a genuinely different symbol, only composite
  // "symbol@timeframe" keys (pineSecurityCacheKey) naming that exact symbol are considered --
  // this chart's own bars/cache entries can never leak into another symbol's request.
  // symbolVal에 따라 lowerTfCache에서 어떤 항목이 후보가 될지가 갈린다: 이 차트 자신의
  // 심볼이면(pineSecurityIsSameSymbol) 예전처럼 평범한 timeframe 문자열 키("1S", "5", "1D" 등)만
  // 본다. 진짜 다른 심볼이면 그 심볼 이름으로 된 "symbol@timeframe" 조합 키(pineSecurityCacheKey)만
  // 본다 — 이 차트 자신의 봉/캐시 항목이 다른 심볼 요청에 섞여 들어갈 일이 없다.
  resolveLowerTfBars(tf, symbolVal){
    if(!this.lowerTfCache) return null;
    const sameSymbol = pineSecurityIsSameSymbol(symbolVal);
    if(sameSymbol){
    ...
    for(const [key, srcBars] of this.lowerTfCache){
      if(!srcBars || !srcBars.length) continue;
      const at = key.indexOf('@');
      const entrySymbol = at >= 0 ? key.slice(0, at) : null;
      const entryTf = at >= 0 ? key.slice(at + 1) : key;
      if(sameSymbol){ if(entrySymbol != null) continue; }
      else if(entrySymbol !== symbolVal) continue;
      const srcSecs = pineTfSeconds(entryTf);
  ...
  buildLowerTfArraySeries(tf, exprNode, symbolVal){
    const n = this.n;
    ...
    const cache = this.resolveLowerTfBars(tf, symbolVal);
    ...
    const save = {
      ...
      curBar: this.curBar, n: this.n, activeSecuritySymbol: this.activeSecuritySymbol,
    };
    ...
    this.n = cn;
    // Same reasoning as buildSecuritySeries: syminfo.mintick/minmove/pricescale inside expr should
    // reflect the requested symbol.
    // buildSecuritySeries와 같은 이유 — expr 안의 syminfo.mintick/minmove/pricescale은 요청받은
    // 심볼을 반영해야 한다.
    this.activeSecuritySymbol = pineSecurityIsSameSymbol(symbolVal) ? pineSymTicker() : symbolVal;
  ...
  buildSecuritySeries(tf, exprNode, lookaheadOn, symbolVal){
    ...
    // boundary entirely to the wrong bucket. In that case, if the host put a usable series into
    // lowerTfCache (see resolveLowerTfBars — either keyed directly for tf, or synthesized on the
    // spot from a finer divisor timeframe already in the cache), that's used directly — its field
    // shape is identical (open/high/low/close/volume/time), so it can be assigned straight into
    // closedBuckets with no conversion. This is also the only source of data at all when a
    // different symbol was requested (see the sameSymbol branch below).
    // 현재 차트 간격이 tf의 약수가 아니면(예: 3분봉 차트에서 "5" 요청) 메인 봉을 그대로 묶는 아래
    // 폴백 경로는 경계에 걸친 봉을 통째로 한쪽 버킷에 잘못 배정한다. 이 경우 호스트가
    // lowerTfCache에 쓸 만한 시리즈를 넣어뒀으면(resolveLowerTfBars 참고 — tf로 직접 키가
    // 잡혀 있거나, 캐시에 있는 더 잘게 쪼갠 timeframe에서 즉석 합성) 그걸 그대로 쓴다 — 필드
    // 모양이 동일해서(open/high/low/close/volume/time) 변환 없이 closedBuckets로 바로 대입
    // 가능하다. 다른 심볼을 요청한 경우엔 이게 유일한 데이터 출처이기도 하다(아래 sameSymbol
    // 분기 참고).
    const preAgg = tf ? this.resolveLowerTfBars(tf, symbolVal) : null;
    ...
    if(preAgg && preAgg.length){
      ...
      // A different symbol was requested, but lowerTfCache has nothing usable for it. Unlike the
      // same-symbol case, there's no safe fallback here — this chart's own bars belong to a
      // different symbol, so bucketing them would silently produce that other symbol's values
      // under the requested symbol's name. Every bar just comes back na, the same
      // graceful-failure behavior request.security_lower_tf() already has when its cache is empty.
      // 다른 심볼을 요청했는데 lowerTfCache에 쓸 만한 데이터가 없다 — 같은 심볼일 때와 달리
      // 여기선 안전한 폴백이 없다(이 차트 자신의 봉은 다른 심볼 것이라, 그걸 묶으면 요청한
      // 심볼 이름으로 조용히 엉뚱한 값을 내보내게 된다). request.security_lower_tf()가 캐시가
      // 비어 있을 때와 똑같이 모든 봉이 그냥 na로 나온다.
      closedBuckets = [];
      for(let i = 0; i < n; i++) bucketOfBar[i] = -1;
    }
    ...
    this.hl2Arr = hl2; this.hlc3Arr = hlc3; this.ohlc4Arr = ohlc4; this.hlcc4Arr = hlcc4;
    this.n = bn;
    // syminfo.mintick/minmove/pricescale evaluated inside expr below should reflect the requested
    // symbol, not whatever was active before this call (e.g. this chart's own symbol, or an outer
    // request.security() call already in progress for yet another symbol).
    // 아래 expr 안에서 평가되는 syminfo.mintick/minmove/pricescale은 이 호출 이전에 활성화돼
    // 있던 심볼(예: 이 차트 자신의 심볼, 또는 이미 진행 중이던 바깥쪽 request.security()의 다른
    // 심볼)이 아니라 지금 요청받은 심볼을 반영해야 한다.
    this.activeSecuritySymbol = sameSymbol ? pineSymTicker() : symbolVal;
