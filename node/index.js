...
module.exports = {
  ...
  //   - lowerTfCache: the Map<key, bars[]> that request.security_lower_tf()/a higher-timeframe
  //     request.security() reads from. If omitted, those calls just return na (the engine is
  //     built to fall back gracefully) -- fetch it from whatever data source you have and fill
  //     it in yourself. For the chart's own symbol, key by a plain timeframe string ("1S", "5",
  //     "1D", ...). For a different symbol, key with securityCacheKey(symbol, timeframe) below --
  //     a request whose symbol argument names a different symbol is only ever served from those
  //     entries, never from this chart's own bars/cache.
  ...
  //     Map<key, bars[]>. 안 주면 해당 호출은 그냥 na를 돌려준다(엔진이 그렇게 우아하게
  //     폴백하도록 되어 있음) — 다른 데이터소스에서 미리 받아와 채워 넣으면 된다. 이 차트
  //     자신의 심볼이면 평범한 timeframe 문자열("1S", "5", "1D" 등)을 키로 쓰고, 다른
  //     심볼이면 아래 securityCacheKey(symbol, timeframe)로 키를 만든다 — symbol 인자가 다른
  //     심볼을 가리키는 요청은 이 차트 자신의 봉/캐시로는 절대 서빙되지 않고 오직 그 항목으로만
  //     서빙된다.
  // 에러는 { pineError: true, line, message } 형태의 일반 객체를 던진다(에러 클래스 아님 — 원본 그대로).
  run: sandbox.runPineScript,
  // Builds the lowerTfCache key for a different symbol's data at a given timeframe (see run()'s
  // lowerTfCache above). Same-symbol entries keep using a plain timeframe string, unchanged.
  // 다른 심볼의 봉을 특정 timeframe으로 lowerTfCache에 넣을 때 쓰는 키를 만든다(위 run()의
  // lowerTfCache 설명 참고). 같은 심볼용 항목은 그대로 평범한 timeframe 문자열을 쓴다.
  securityCacheKey: sandbox.pineSecurityCacheKey,
  ...
  // The current symbol that syminfo.* refers to, e.g. { coin: 'BTC', isSpot: false,
  // exchangePrefix: 'BINANCE' }. exchangePrefix feeds syminfo.prefix and the "EXCHANGE:" part of
  // syminfo.tickerid (both empty by default). If never called, the symbol is treated as empty.
  // syminfo.*가 참조하는 현재 심볼. { coin: 'BTC', isSpot: false, exchangePrefix: 'BINANCE' }
  // 처럼. exchangePrefix는 syminfo.prefix와 syminfo.tickerid의 "거래소:" 부분에 쓰인다(기본은
  // 둘 다 빈 문자열). 안 부르면 빈 심볼 취급.
  setSymbol: sandbox.pineSetSymbol,
  // Overrides the syminfo.mintick/minmove/pricescale heuristic for a specific symbol:
  // (symbol: string) => number | null | undefined. `symbol` is this chart's own syminfo.ticker/
  // tickerid when evaluating normally, or the exact string a script passed as request.security()'s
  // symbol argument while evaluating that call's expression for a different symbol. Return a
  // positive number to use as the tick size, or null/undefined to fall back to the built-in
  // 5-significant-figure approximation.
  // syminfo.mintick/minmove/pricescale 근사를 특정 심볼에 대해 대체한다:
  // (symbol: string) => number | null | undefined. `symbol`은 평소엔 이 차트 자신의
  // syminfo.ticker/tickerid이고, request.security()가 다른 심볼의 식을 평가하는 동안엔
  // 스크립트가 그 호출의 symbol 인자로 넘긴 문자열 그대로다. 양수를 돌려주면 틱 사이즈로
  // 쓰고, null/undefined면 내장된 유효숫자 5자리 근사로 폴백한다.
  setMintickResolver: sandbox.pineSetMintickResolver,
